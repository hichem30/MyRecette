  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-serif text-2xl font-bold text-neutral-900">{t("admin.dashboard")}</h1>
        </div>
        <Link
          href="/admin/products/new"
          className="inline-flex items-center gap-1.5 rounded-md bg-recette-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-recette-700"
        >
      ...
      {!isSupabaseConfigured() && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          <strong>{t("admin.supabaseNotConfigured")}</strong> {t("admin.supabaseConfigInstructions")}
        </div>
      )}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          sub={data && data.outOfStock > 0 ? `${data.outOfStock} ${t("admin.outOfStock")}` : t("admin.liveInCatalog")}
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-xl border border-neutral-200 bg-white shadow-sm">
          <header className="flex items-center justify-between border-b border-neutral-100 px-5 py-4">
            <div>
              <h2 className="text-sm font-bold text-neutral-900">{t("admin.recentOrders")}</h2>
              <p className="text-xs text-neutral-500">{t("admin.mostRecentCustomerPurchases")}</p>
            </div>
            <Link
              href="/admin/orders"
              className="inline-flex items-center gap-1 text-xs font-semibold text-recette-700 hover:text-recette-800"
            >
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-neutral-800">
                      {o.customer_email ?? t("common.guestCheckout")}
                    </p>
        ...
        <div className="rounded-xl border border-neutral-200 bg-white shadow-sm">
          <header className="flex items-center justify-between border-b border-neutral-100 px-5 py-4">
            <div>
              <h2 className="text-sm font-bold text-neutral-900">{t("admin.recentMessages")}</h2>
              <p className="text-xs text-neutral-500">{t("admin.customerSupportInbox")}</p>
            </div>
            <Link
              href="/admin/messages"
              className="inline-flex items-center gap-1 text-xs font-semibold text-recette-700 hover:text-recette-800"
                ...
                >
                  <span
                    className={`mt-1 inline-block h-2 w-2 flex-none rounded-full ${
                      m.read ? "bg-neutral-200" : "bg-recette-600"
                    }`}
                    aria-hidden
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-neutral-500">
                      {m.subject || t("common.noSubject")} · {timeAgo(m.created_at)}
                    </p>
                  </div>
                </Link>
              ))
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <QuickAction href="/admin/products" Icon={Package} title={t("admin.products")} body={t("admin.addEditRestock")} />
          <QuickAction href="/admin/promos" Icon={Tag} title={t("admin.promos")} body={t("admin.manageDiscountCodes")} />
          <QuickAction href="/admin/orders" Icon={ShoppingBag} title={t("admin.orders")} body={t("admin.updateShippingStatus")} />
          <QuickAction href="/admin/staff" Icon={Users} title={t("admin.staff")} body={t("admin.inviteTeamMembers")} />
        </div>
      </section>
    </div>
  );
...
function QuickAction({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-3 rounded-xl border border-neutral-200 bg-white p-4 transition hover:border-recette-500 hover:shadow-md"
    >
      <span className="inline-flex h-10 w-10 flex-none items-center justify-center rounded-md bg-neutral-100 text-neutral-600 group-hover:bg-recette-50 group-hover:text-recette-700">
        <Icon className="h-4 w-4" />
      </span>
      ...
      <ArrowRight className="ml-auto h-4 w-4 flex-none text-neutral-300 group-hover:text-recette-600" />
    </Link>
  );
}
