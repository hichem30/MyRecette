...
export default function AdminLoginPage() {
  ...
  async function provider(p: "google") {
    setError(null);
    if (!isSupabaseConfigured()) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-100 p-6">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
        <Link href="/" className="text-xs text-neutral-500 hover:text-recette-700">← Back to store</Link>
        ...
        <form onSubmit={signIn} className="space-y-3">
          <div className="flex items-center gap-2 rounded-md border border-neutral-300 bg-white px-3 py-2 focus-within:border-recette-600">
            ...
            <input name="email" type="email" required placeholder="admin@myrecette.com" className="flex-1 bg-transparent text-sm outline-none" />
          </div>
          <div className="flex items-center gap-2 rounded-md border border-neutral-300 bg-white px-3 py-2 focus-within:border-recette-600">
