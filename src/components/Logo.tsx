...
export function Logo({ variant = "dark" }: { variant?: "dark" | "light" }) {
  ...
  return (
    <Link href="/" className="flex items-center gap-2 group min-w-0" aria-label="Red Barn Western Market home">
      ...
      <span className="flex min-w-0 flex-col leading-tight">
        <span className={`font-serif text-base sm:text-lg font-bold whitespace-nowrap ${text}`}>Red Barn</span>
        <span className={`text-[9px] sm:text-[10px] tracking-widest whitespace-nowrap ${sub}`}>WESTERN MARKET</span>
      </span>
    </Link>
  );
}
